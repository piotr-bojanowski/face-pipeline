...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if (mxIsComplex(prhs[0]) ||	!mxIsLogical(prhs[0]) || mxGetNumberOfDimensions(prhs[0]) != 2)
		mexErrMsgTxt("argument 1 (M) must be a logical matrix");
	int h = mxGetM(prhs[0]), w = mxGetN(prhs[0]);
	const mxLogical *M = mxGetLogicals(prhs[0]);

	if (mxIsComplex(prhs[1]) ||	!mxIsSingle(prhs[1]) || mxGetM(prhs[1]) != h || mxGetN(prhs[1]) != w)
		mexErrMsgTxt("argument 2 (XX) must be asingle matrix");
	const float *XX = (const float *) mxGetData(prhs[1]);
	...
	const float *YY = (const float *) mxGetData(prhs[2]);
	...
	const float *XY = (const float *) mxGetData(prhs[3]);
	...
	float mineigval = (float) mxGetScalar(prhs[4]);
