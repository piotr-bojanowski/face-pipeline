...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	TrackingContext *tc = ParseTrackingContext(prhs[0], err);
	if (err)
		mexErrMsgTxt(szErrMsgs[err]);

	IXYPyramid *oldpyr = ParsePyramid(prhs[1], err);
	if (err)
	{
		delete tc;
		mexErrMsgTxt(szErrMsgs[err]);
	...
	if (!mxIsSingle(prhs[2]) || mxIsComplex(prhs[2]) || mxGetNumberOfDimensions(prhs[2]) != 2 ||
		mxGetM(prhs[2]) != 3)
	{
		delete tc;
	...
	if (!mxIsSingle(prhs[3]) || mxIsComplex(prhs[3]) || mxGetNumberOfDimensions(prhs[3]) != 2 ||
		mxGetM(prhs[3]) != 3 || mxGetN(prhs[3]) != nf)
	{
		delete tc;
	...
	const float *P = (const float *) mxGetData(prhs[2]);
	const float *Pp = (const float *) mxGetData(prhs[3]);
